...
function TooltipOverlay() {
  ...
  // Note: Arrow middleware is disabled to avoid ref conflicts
  // const arrowElementRef = React.useCallback((element: SVGSVGElement | null) => {
  //   arrowRef.current = element;
  // }, []);

  // Create middleware with stable references
  const middleware = React.useMemo(() => [
    ...
    flip(),
    shift({ padding: 8 }),
    // Temporarily disable arrow middleware to fix ref access error
    // floatingArrow({
    //   element: arrowElementRef,
    //   padding: 8
    // }),
  ], [rendered.data?.sideOffset, rendered.data?.alignOffset]);
  ...
  // Set up floating element ref
  const floatingRef = React.useCallback((node: HTMLElement | null) => {
